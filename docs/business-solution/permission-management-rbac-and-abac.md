...
>
> Casbin 中有个[条件角色管理器](https://casbin.org/zh/docs/rbac-with-conditions), 定义**什么条件下可以为用户赋予某个角色**，其实并不是适用于实现 RBAC2，这个角色的授予操作是在鉴权过程中授予的，虽然官方说是定义临时角色的，不过授予之后角色会一直存在，即便条件不再符合也不会自动删除。
